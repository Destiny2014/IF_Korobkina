...
public class CharacterSteps {
    ...
    @Step("Проверка полученного ответа получения персонажа по имени '{name}' и id '{id}'")
    ...
    @Step("Проверка полученного ответа получения последнего персонажа")
    public String getLastCharacter(JsonPath body) {
        List<String> characters = body.get("characters");
        return characters.get(characters.size() - 1);
    }

    @Step("Проверка полученного ответа получения персонажа по id")
